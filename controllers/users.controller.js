...
exports.saveUser = async (req, res) => {
	...
	// check db password fields is null then set the new password
	const checkPwdSql = "SELECT * FROM users WHERE mobile=?";
	connection.query(checkPwdSql, [mobile], async (error, results, fields) => {
		if (error) res.status(202).json({ status: false, result: error });
		if (
			results?.length > 0 &&
			(results[0]?.password === null || results[0].password === "")
		) {
			connection.query(sql, data, (error) => {
				res.status(200).json({ status: true, result: authJwt });
			});
		} else {
			//check for existing user
